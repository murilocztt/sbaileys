...
,
  counters as (
    select
      (select avg(supportTime) from tracking where supportTime > 0) avgSupportTime,
      (select avg(waitTime) from tracking where waitTime > 0) avgWaitTime,
      (
        select count(distinct t1.id)
        from Tickets t1
         inner join tracking tt1 on t1.id = tt1.ticketId
      ...
      (select count(id) from tracking where finished) supportFinished,
      (
      ...
      (
        select 
          (100*count(tt.id))/NULLIF((select count(id) total from tracking where rated= 1),0)
        from tracking  tt
        left join UserRatings ur on ur.ticketId = tt.ticketId
        where tt.rated =true 
          and ur.rate > 8
      ...
      (
        select 
          nullif((100*count(tt.ID))/NULLIF((select count(id) total from tracking where rated= 1),0),0)
        from tracking  tt
        left join UserRatings ur on ur.ticketId = tt.ticketId
        where tt.rated = true 
          and ur.rate < 7
      ...
      (
        select sum(nps.promoter-nps.detractor)   from (
          (select 
           (100*count(tt.id))/NULLIF((select count(id) total from tracking where rated= 1),0) promoter
       , 0 detractor
                    from tracking tt
                    left join UserRatings ur on ur.ticketId = tt.ticketId
                where tt.rated =true 
          ...
          union			
           select 
          0,nullif((100*count(tt.ID))/NULLIF((select count(id) total from tracking where rated= 1),0),0)
        from tracking  tt
        left join UserRatings ur on ur.ticketId = tt.ticketId
        where tt.rated = true 
          and ur.rate < 7)) nps
  ...
  attedants as (
      ...
      from Users u1
      left join tracking t on t.userId = u1.id
